...
#get proportion of p value less than .05
...
aov_out<-aov(DV ~ IV + Error(S), data=sphericity)
summary(aov_out)
...
#Sphericity happens when there are uneven covariance between levels. In other words, each subject should should show similar effect across the levels.  Here, we do not see that there is a sphericity as the lines representing the results of each subject are very different--lines cross at different points. 
```
